...
export default function SpeakingCoach() {
  ...
  function AudioInput({ questionKey, existingText, onClear }) {
    ...
    if (isTranscribing) return (
      <div className="flex items-center gap-2 text-gray-500 text-sm bg-gray-100 rounded-xl px-4 py-3">
    ...
    if (existingText) return (
      <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-3">
        <div className="flex items-start gap-2 mb-2">
          ...
          <p className="text-gray-600 text-sm italic">"{existingText}"</p>
        </div>
        <button onClick={onClear} className="text-gray-500 text-xs hover:text-gray-600 transition-colors">Redo</button>
      </div>
    )

    const mins = String(Math.floor(recordingSecs / 60)).padStart(2, '0')
    const secs = String(recordingSecs % 60).padStart(2, '0')
      ...
      <div className="flex items-center gap-3 bg-gray-100 rounded-xl px-4 py-3">
        <button
          ...
          className={isRecording
            ? 'flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-red-500 text-white'
            : 'flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-gray-200 text-gray-600 hover:bg-gray-300'
          }
        >
        ...
        {isRecording ? (
          <div className="flex items-center gap-2">
            ...
            <span className="text-red-600 font-mono text-sm font-semibold">{mins}:{secs}</span>
            <span className="text-gray-500 text-xs">Recording...</span>
          </div>
        ) : (
          <>
            <span className="text-gray-600 text-sm">or</span>
            <label className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-gray-200 text-gray-600 hover:bg-gray-300 cursor-pointer">
  ...
  if (fetching && phase === 'loading') return (
    <div className="flex items-center justify-center h-96">
      <div className="text-center">
        ...
        <p className="text-gray-500">Loading your speaking session...</p>
      </div>
    </div>
  )
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-4 mb-6">
          ...
          {memories.map((mem, i) => (
            <p key={i} className="text-gray-500 text-sm">
              {mem.memory_type === 'weakness' ? '⚠️' : '✅'} <span className="text-gray-600">{mem.skill}:</span> {mem.memory_text}
            </p>
          ))}
        </div>
      )}
      ...
      {phase === 'session' && prompt && (
        <div className="space-y-6">
              ...
              return (
                <div key={partKey} className="flex items-center gap-2">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                    isDone ? 'bg-green-500 text-gray-900' : isActive ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-500'
                  }`}>{i + 1}</div>
          ...
          {currentPart === 'part1' && (
            <div className="space-y-4">
              <div className="bg-white border border-gray-200 rounded-2xl p-5">
                <h2 className="text-lg font-semibold text-gray-900 mb-1">Part 1 — Personal Questions</h2>
                <p className="text-gray-500 text-sm mb-4">Answer each question naturally. Aim for 2-3 sentences.</p>
                <div className="space-y-5">
                  {prompt.part1.questions.map((q, i) => (
                    <div key={i}>
                      <p className="text-gray-600 text-sm mb-2"><span className="text-gray-500 mr-1">Q{i+1}.</span>{q}</p>
                      <AudioInput
                        questionKey={'p1_' + i}
          ...
          {currentPart === 'part2' && (
            <div className="space-y-4">
              <div className="bg-white border border-gray-200 rounded-2xl p-5">
                <h2 className="text-lg font-semibold text-gray-900 mb-1">Part 2 — Long Turn</h2>
                <p className="text-gray-500 text-sm mb-4">Read the cue card, prepare for 1 minute, then speak for 1-2 minutes.</p>
                <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 mb-4">
                  <p className="text-blue-300 text-sm font-medium mb-2">📋 Cue Card</p>
                  <p className="text-gray-600 text-sm whitespace-pre-line">{prompt.part2.cue_card}</p>
                </div>
                <AudioInput questionKey="p2" existingText={part2Response} onClear={() => setPart2Response('')} />
              </div>
              <div className="flex gap-3">
                <button onClick={() => setCurrentPart('part1')} className="px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-xl transition-colors">← Back</button>
                <button onClick={() => setCurrentPart('part3')} disabled={!p2Done}
                  className="flex-1 py-3 bg-brand-500 hover:bg-brand-600 disabled:opacity-40 disabled:cursor-not-allowed text-gray-900 font-semibold rounded-xl transition-colors">
                  Continue to Part 3 →
                </button>
              </div>
            </div>
          ...
          {currentPart === 'part3' && (
            <div className="space-y-4">
              <div className="bg-white border border-gray-200 rounded-2xl p-5">
                <h2 className="text-lg font-semibold text-gray-900 mb-1">Part 3 — Discussion</h2>
                <p className="text-gray-500 text-sm mb-4">Give extended answers. Support your views with reasons and examples.</p>
                <div className="space-y-5">
              ...
              <div className="flex gap-3">
                <button onClick={() => setCurrentPart('part2')} className="px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-xl transition-colors">← Back</button>
                <button onClick={handleSubmit} disabled={submitting || !p3Done}
                  className="flex-1 flex items-center justify-center gap-2 py-3 bg-brand-500 hover:bg-brand-600 disabled:opacity-40 disabled:cursor-not-allowed text-gray-900 font-semibold rounded-xl transition-colors">
      ...
      {phase === 'results' && results && (
        <div className="space-y-6">
          <h2 className="text-xl font-bold text-gray-900">Speaking Test Results</h2>

          {ttsUrl ? (
            <div className="bg-white border border-gray-200 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-3">
                <Volume2 className="w-5 h-5 text-brand-600" />
                <span className="text-gray-900 font-medium">Listen to your examiner feedback</span>
              </div>
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-4 flex items-center gap-3">
              ...
              <span className="text-gray-500 text-sm">Generating spoken feedback...</span>
            </div>
          )}

          <div className="bg-white border border-gray-200 rounded-2xl p-5">
            <h3 className="text-base font-semibold text-gray-900 mb-3">💬 Examiner Feedback</h3>
            <p className="text-gray-600 leading-relaxed">{results.feedback_text}</p>
          </div>

          {results.scores && (
            <div className="bg-white border border-gray-200 rounded-2xl p-5">
              <h3 className="text-base font-semibold text-gray-900 mb-4">Band Scores</h3>
              ...
              {results.scores.overall_band && (
                <div className="text-center py-3 bg-brand-500/10 border border-brand-500/30 rounded-xl">
                  <p className="text-gray-500 text-sm">Overall Band</p>
                  <p className="text-brand-600 text-4xl font-bold">{results.scores.overall_band}</p>
                </div>
              )}
            </div>
          )}
